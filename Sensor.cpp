...
void Sensor::setupRadio() {
	...
	RF24::openReadingPipe(BROADCAST_PIPE, BASE_RADIO_ID + BROADCAST_ADDRESS);
        RF24::printDetails();
        fprintf(stderr,"Radio setup complete-");
}
...
boolean Sensor::sendWrite(uint8_t dest, message_s message, int length) {
	...
	int retry = 5;
	do {
		ok = RF24::write(&message, min((const unsigned int) MAX_MESSAGE_LENGTH, (const unsigned int) (sizeof(message.header) + length)));
	}
	while ( !ok && --retry );
	RF24::startListening();
...
boolean Sensor::messageAvailable() {
	if (RF24::available()) {
		...
		debug(PSTR("Mess crc %s.\n"),ok?"ok":"error");
                debug(PSTR("header.to=%d, radioId=%d\n"),msg.header.to, radioId);
